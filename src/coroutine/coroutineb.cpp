...
struct sync {
  ...
  void resum_hand() {
    std::cout << "sync resum_hand" << std::endl;
  ...
  struct promise_type {
    ...
    auto return_value(T v) {
      Trace t;
      std::cout << "Syn Got an answer of " << v << std::endl;
      value = v;
      return std::suspend_never{};
    }
    auto final_suspend() noexcept {
      Trace t;
      std::cout << "syn promise Finished the coro" << std::endl;
      return std::suspend_always{};
    }
...
struct lazy {
  ...
  struct promise_type {
    ...
    auto initial_suspend() {
      Trace t;
      std::cout << "Started the lazy coroutine, don't stop now!" << std::endl;
      return std::suspend_never{};
    ...
    auto yield_value(T v) noexcept {
      value = std::move(v);
      std::cout << "Yield an answer of " << value << std::endl;
      return std::suspend_always{};
    }
  };
  bool await_ready() {
...
sync<int> reply() {
  std::cout << ">1 readStarted await_answer" << std::endl;
  auto a = co_await read_data();
  std::cout << "Data we got is " << a << std::endl;

  std::cout << ">2 read Started await_answer" << std::endl;
  auto b = co_await read_data();
  std::cout << "Data we got is " << b << std::endl;
  std::cout << ">1 write Started write await_answer" << std::endl;
...
int main() {
  std::cout << "Start main()\n";
  auto a = reply();
  std::cout << "Resum on main()\n";
  a.resum_hand();
  std::cout << "write result is " << a.get() << std::endl;
