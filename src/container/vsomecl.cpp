...
public:
  client_sample(bool _use_tcp, bool _be_quiet, uint32_t _cycle)
      : app_(vsomeip::runtime::get()->create_application()),
        ...
        use_tcp_(_use_tcp), be_quiet_(_be_quiet), cycle_(_cycle),
        running_(true), blocked_(false), is_available_(false),
        sender_(std::bind(&client_sample::run, this)) {
    pthread_setname_np(pthread_self(), "client_main");

    auto h = sender_.native_handle();
    pthread_setname_np(h, "sender");
  }
  ...
  bool init() {
    ...
    std::string msg = "nihao-wodme";
    std::vector<vsomeip::byte_t> its_payload_data;
    its_payload_data.assign(msg.begin(), msg.end());
  ...
  void on_message(const std::shared_ptr<vsomeip::message> &_response) {
    std::cout << "Received a response from Service [" << std::setw(4)
              ...
              << "] " << _response->get_payload()->get_data() << std::endl;
    ...
    while (running_) {
      {
        ...
        if (is_available_) {
          app_->send(request_);
          std::cout << "Client/Session [" << std::setw(4) << std::setfill('0')
